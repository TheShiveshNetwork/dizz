...
func (a *Analyzer) Analyze(files []string) (*signals.SignalSet, error) {
	...
	for _, filePath := range files {
		...
		// Extract signals
		a.extractDefinitions(file, filePath, sigSet)
		a.extractCalls(file, filePath, sigSet)
		a.extractImports(file, filePath, sigSet)
...
func (a *Analyzer) extractDefinitions(file *ast.File, filePath string, sigSet *signals.SignalSet) {
...
func (a *Analyzer) extractCalls(file *ast.File, filePath string, sigSet *signals.SignalSet) {
	ast.Inspect(file, func(n ast.Node) bool {
...
func (a *Analyzer) extractImports(file *ast.File, filePath string, sigSet *signals.SignalSet) {
