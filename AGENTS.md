...
16. **Site build copies from root-level source dirs**: The WASM site at `site/` is generated by `scripts/build-site.sh`, which copies assets from root-level directories into `site/public/`. Never put content directly in `site/` subdirectories that is meant to be served — put it at the project root and let the build script copy it. Currently these directories are synced:
    - `scripts/install.sh` and `scripts/install.ps1` → `site/public/` (end-user download scripts)
    - `internal/defaults/agent-skills/` → `site/public/agent-skills/` (canonical SKILL.md for agent discovery)
    - `site/assets/` → `site/public/assets/` (images, logos)
    If you need a new file to appear in the built site, place it at the project root and add a copy step in `scripts/build-site.sh`. Do not create new subdirectories under `site/` for source content.
